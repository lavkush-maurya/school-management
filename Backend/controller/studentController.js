...
exports.fetchAllStudents = async (req, res) => {
  ...
  try {
    ...
    const students = Object.keys(searchCriteria).length === 0 
      ? await Student.find() 
...
exports.createStudent = [
  ...
  body('aadharCard').isLength({ min: 12, max: 12 }).withMessage('Aadhar Card must be 12 digits'),
  body('fees').isNumeric().withMessage('Fees must be a number'),
  body('yearOfAdmission').isNumeric().withMessage('Year of Admission must be a number'),
  ...
  async (req, res, next) => {
    ...
    const existingStudent = await Student.findOne({ rollNumber, class: studentClass });
    if (existingStudent) {
      return res.status(400).json({ errors: [{ msg: 'Roll Number must be unique within the same class.' }] });
    }
...
exports.updateStudent = async (req, res) => {
  ...
  try {
    ...
    const rollNumberChanged = rollNumber && rollNumber !== studentToUpdate.rollNumber;
